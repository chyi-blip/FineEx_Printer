package com.fineex.printer.esc;
...
public class Barcode extends BaseESC {
     ...
     * 构造函数
     */
    public Barcode(Port port, FineExPrinter.PRINTER_TYPE printer_type) {
     ...
     * Code128自动计算校验和
     */
    public boolean code128_auto_drawOut(FineExPrinter.ALIGN align, ESC.BAR_UNIT unit, int height, ESC.BAR_TEXT_POS pos, ESC.BAR_TEXT_SIZE size, String str) {
        Code128 code128 = new Code128(str);
        byte[] buf = code128.encode_data;
        if (buf == null)
            return false;
    ...
    public boolean code128_auto_printOut(FineExPrinter.ALIGN align, ESC.BAR_UNIT unit, int height, ESC.BAR_TEXT_POS pos, ESC.BAR_TEXT_SIZE size, String str) {
        if (!code128_auto_drawOut(align, unit, height, pos, size, str))
            return false;
        enter();
        if (!setAlign(FineExPrinter.ALIGN.LEFT))
            return false;
        return true;
    }
}
